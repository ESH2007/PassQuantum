...
func (p *SVPicker) CreateRenderer() fyne.WidgetRenderer {
	...
	border.StrokeColor = ColorBorderCyan
...
func buildColorPickerPanel(_ string, st *colorPickerState) fyne.CanvasObject {
	selectorLabel := CreateLabel("Selector", 9, ColorTextSec, false)
	leftCol := container.NewVBox(selectorLabel, st.svPicker)

	hexLabel := CreateLabel("HEX", 9, ColorTextSec, false)
...
func ShowColorPersonalizationDialog(w fyne.Window, fyneApp fyne.App, appState *AppState) {
	...
	rebuildTabs = func() {
		tabStrip.Objects = []fyne.CanvasObject{
			CreateTabButton(tabNames[0], selected == 0, func() { selected = 0; rebuildTabs() }),
			CreateTabButton(tabNames[1], selected == 1, func() { selected = 1; rebuildTabs() }),
			CreateTabButton(tabNames[2], selected == 2, func() { selected = 2; rebuildTabs() }),
		}
		tabStrip.Refresh()
	...
	titleLabel := CreateLabel("MANUAL COLOR PERSONALIZATION", 14, ColorAccentCyan, true)
	divider := canvas.NewRectangle(ColorBorderCyan)
