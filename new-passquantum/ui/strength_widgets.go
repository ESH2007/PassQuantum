...
func NewStrengthBar() fyne.CanvasObject {
	...
	titleLabel := canvas.NewText("STRENGTH ANALYSIS", ColorAccentCyan)
	titleLabel.TextStyle = fyne.TextStyle{Bold: true}
	titleLabel.TextSize = 11

	scoreLabel := canvas.NewText("Very Weak  🔓", ColorTextPrimary)
	scoreLabel.TextStyle = fyne.TextStyle{Bold: true}
	scoreLabel.TextSize = 12

	crackLabel := canvas.NewText("Crack time: Instantly", ColorTextSecondary)
	crackLabel.TextSize = 11

	issuesContainer := container.NewVBox(newStrengthText("Start typing to analyze password strength.", ColorTextSecondary, 11, false))
...
func NewIssuesList(issues []strength.Issue) fyne.CanvasObject {
	if len(issues) == 0 {
		return newStrengthText("No major issues detected.", ColorTextSecondary, 11, false)
	}
	...
	for _, issue := range issues {
		...
		label := newStrengthText(indicator+" "+issue.Message, ColorTextSecondary, 11, false)
...
func NewEasterEggPanel(rules []strength.EasterEggRule) fyne.CanvasObject {
	intro := newStrengthText(strength.EasterEggIntroMessage(), ColorTextPrimary, 11, true)
...
func storedVaultPasswords(appState *AppState) []string {
	...
	for _, entry := range entries {
		...
		sharedSecret, err := Decapsulate(entry.KyberCiphertext, appState.privateKey)
		if err != nil {
			continue
		}
		plaintext, err := DecryptAES256GCM(entry.Nonce, entry.Ciphertext, sharedSecret)
		if err != nil {
			continue
		}
