...
func ShowAppInformation(title, message string, w fyne.Window) {
	showAppDialog(w, title, message, "i", ColorAccentCyan, []appDialogAction{
		{label: "OK", primary: true},
	})
}

func ShowAppWarning(title, message string, w fyne.Window) {
	showAppDialog(w, title, message, "!", ColorWarning, []appDialogAction{
		{label: "OK", primary: true},
	})
}
...
func ShowAppError(err error, w fyne.Window) {
	...
	showAppDialog(w, "Error", err.Error(), "!", ColorDanger, []appDialogAction{
		{label: "OK", primary: true},
	})
}
...
func showAppDialog(w fyne.Window, title, message, glyph string, tone color.NRGBA, actions []appDialogAction) {
	...
	titleLabel := CreateLabel(title, 16, ColorTextPrimary, true)
	messageLabel := widget.NewLabel(message)
	messageLabel.Wrapping = fyne.TextWrapWord

	iconCircle := canvas.NewCircle(color.NRGBA{R: tone.R, G: tone.G, B: tone.B, A: 35})
	iconSymbol := CreateLabel(glyph, 44, color.NRGBA{R: tone.R, G: tone.G, B: tone.B, A: 170}, true)
	...
	for _, action := range actions {
		...
		if a.primary {
			btn = CreateNeonButton(a.label, onTap, 95, 38)
		} else {
			btn = CreateSecondaryButton(a.label, onTap, 95, 38)
		}
	...
	card := CreateCard(content, 440, 0, true)
