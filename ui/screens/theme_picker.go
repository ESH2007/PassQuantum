...
func showThemePicker(a fyne.App, w fyne.Window, onApplied func()) {
	widgets.PickImageFile("Select Theme Image", func(path string) {
		f, err := os.Open(path)
		if err != nil {
			widgets.ShowAppError(fmt.Errorf("could not open image: %w", err), w)
			return
		}
		...
		if decodeErr != nil {
			widgets.ShowAppError(fmt.Errorf("could not decode image: %w", decodeErr), w)
			return
		}
		...
		if len(clusters) == 0 {
			widgets.ShowAppError(fmt.Errorf("could not extract dominant colors from image"), w)
			return
		}
