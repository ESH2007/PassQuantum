...
$EntryPoint   = Join-Path $ProjectRoot "python\face_guard.py"
...
$RequiredFiles = @(
    $EntryPoint,
    (Join-Path $PythonDir "geometric_encoder.py"),
    (Join-Path $PythonDir "liveness_detector.py"),
    (Join-Path $PythonDir "face_authenticator.py"),
